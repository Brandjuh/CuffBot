...
| **Commands** | `!channellist` group (admin, S70 — actions + settings merged; alias `!channellist`) |
...
| **Default channel** | None — the owner has not named one; post it once with `!channellist post #…` |

## Commands

### !channellist (admin — Manage Server; S70 group command — actions + settings in one group, alias `!channellist`)

Bare `!channellist` = the settings view. Subcommands:

| Subcommand | Does |
|---|---|
| `!channellist post [#channel]` | (Re)post the full list, replacing any previous one — optionally into a new channel |
| `!channellist update` | Refresh, **editing the existing messages in place** when possible |
| `!channellist remove` | Delete the posted list |
| `!channellist role <@role>` | Only channels THIS role can see are listed (default @everyone) |
| `!channellist everyone` | Reset visibility to @everyone |
| `!channellist header <text…>` | Intro text (greedy); `default` restores |
| `!channellist emoji <emoji>` | Decorates category headers; `none` removes |
| `!channellist color <hex>` | Embed color like `#5865f2`; `default` restores |
| `!channellist voice <on\|off>` | Include voice and stage channels |
| `!channellist autoupdate <on\|off>` (alias `auto-update`) | Refresh automatically when channels change |
| `!channellist ignore <#channel>` | Hide a channel **or a whole category** from the list |
| `!channellist unignore <#channel\|id>` | Show it again — accepts a raw id, so entries for deleted channels can be cleaned up |
...
- A per-guild lock serializes refreshes (a manual `!channellist update` and the debounced auto-refresh can never interleave their fetch/edit/send sequences).
...
- **Manual (live server) checklist:**
  1. `!channellist post #directory` → the full list appears, grouped per category.
  ...
  5. `!channellist ignore #staff-category` → the whole category disappears from the list.
  6. `!channellist role @Member` → the list now only shows what @Member can see.
  7. `!channellist remove` → list deleted, auto-updates stop.

## Troubleshooting

| Symptom | Likely cause | Fix |
|---|---|---|
| "No list channel set yet" | Nothing configured | `!channellist post #…` |
| List never auto-updates | Auto-update off, or no list posted | `!channellist` shows both |
| A private channel shows up | The visibility role can see it | `!channellist role` (or fix the channel permissions) |
...
| S70 | `!channellist` and `!channellist` merged into ONE group (M17.2; the config name stays as an alias): post/update/remove + role/everyone/header/emoji/color/voice/autoupdate/ignore/unignore (unignore accepts raw ids for deleted channels — replaces `unignore-id`). |
