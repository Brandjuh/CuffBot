...
| **Commands** | `!starboard` group (admin, S70; alias `!starboard-config`) |
...
### !starboard (admin — Manage Server; S70 group command, alias `!starboard-config`)

Bare `!starboard` = the status view (also shows how many messages have been boarded). Subcommands:

| Subcommand | Does |
|---|---|
| `!starboard on` / `!starboard off` | Board on/off |
| `!starboard channel <#channel>` | Where starred messages are reposted |
| `!starboard threshold <1–25>` | Stars needed to board a message (default 3) |
| `!starboard emoji <emoji>` | S25 — a unicode emoji like 🌟/🍩, **or a custom server emoji** picked from the emoji picker so it looks like `<:name:id>`; custom emoji match on their ID |
...
- **Manual (live server) checklist:**
  1. `!starboard channel #commendations` then `!starboard threshold 2`.
  ...
  7. `!starboard` → "Boarded so far" increments.

## Troubleshooting

| Symptom | Likely cause | Fix |
|---|---|---|
| Stars do nothing | No channel set / disabled / threshold not reached | `!starboard` shows all three |
...
| S70 | Converted to the `!starboard` group (M17.2; alias `!starboard-config`): on/off, channel, threshold, emoji. |
