...
| `!rank-exclude` | Add/remove a non-rank role from the ladder | `<role> [action:add\|remove]` | Manage Server | `!rank-exclude @DJ action:add` |
...
- **Failure modes (all specific):** target not in the guild; ladder not configured (→ points at `!rank-setup`); already at the top/bottom; `to` not a promotion/demotion; `to` not a rank; bot lacks Manage Roles; a rank role sits at/above the bot's role (→ move CuffBot's role up).
