...
|---|---|---|
| Bot sits in the voice channel but writes nothing | Before S136: a self-update restarted the bot mid-session and nothing resumed it | Update to S136+; the boot sweep resumes or visibly leaves. If it still happens, check `!transcribe` for the Live line and `journalctl -u cuffbot` for Groq-limit warnings |
...
| Session | Change |
|---|---|
| S136 | **Live sessions survive the self-updating bot.** The bot restarts itself on every merged PR (S127); sessions were RAM-only and auto-join only fires when someone ENTERS a channel — so each update killed a running transcription while Discord kept showing the bot in the voice channel (owner: "de bot is wel in het kanaal"). Now: a **boot sweep** reconciles Discord's picture with reality — a lingering voice state with people present resumes (announced: "I restarted for an update and picked the session back up"), a lingering state in an empty/disabled room is left visibly, and people already sitting in a channel are joined per the normal auto-join gate (fullest room wins). A **shutdown hook** (new loader-level mechanism) drains held audio and leaves the channel cleanly on the update-exit and on SIGTERM, so no ghost. Also: a daily-budget refusal mid-session is now logged instead of silently dropped. |
| S134 | **Transcript stamps are Discord timestamps.** Each line was prefixed with `` `14:32` `` — UTC, i.e. the Pi's timezone, and inside a code span. It is now `<t:…:t>`, which renders the same clock time in the reader's own zone. `:t` and not `:R` deliberately: relative is right for a countdown and useless down a transcript, where every line would read "a minute ago". |
