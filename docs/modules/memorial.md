...
- **Options:** `enabled` (bool), `channel` (shared fallback channel), `preview` (bool — fetches each feed **now** and shows its latest entry ephemerally; nothing is posted, nothing is marked seen), `officers-channel` + `firefighters-channel` (S60 — each feed's OWN channel; wins over `channel:`; officers defaults to `451095508560379934` since S61), `probe` (S61 — fetch ANY candidate feed URL live from the bot host and show item count + newest three titles/links; posts nothing, commits nothing), `officers-role` + `firefighters-role` (S62 — per-feed ping role, overriding the committed defaults; the status view warns with the exact fixing option when a configured role no longer exists in the guild). All channel pickers take text or announcement channels.
