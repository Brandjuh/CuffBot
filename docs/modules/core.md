...
> **A stale check is labelled stale (S129).** "Last check: 4 minutes ago — Already on the latest version" printed under "2 commits behind" reads as a contradiction, even when both are true at their own timestamps. The line now says *"it was up to date then; the commits above landed after it"* and names the next check. It fires **only** for a stale `up-to-date` — a failed last run is never softened, because behind-because-red is a different fact from behind-because-new.
...
| Session | Change |
|---|---|
| S136 | **The update-restart exits gracefully.** Before its exit (and on SIGTERM from `systemctl stop/restart`), the process now runs every module's optional `shutdown()` hook (bounded at 4 s) and closes the gateway with `client.destroy()`, so Discord's picture of the bot — voice states included — matches reality instead of showing a ghost. First consumer: transcribe's live voice sessions. |
| S129 | **Verified live on the Pi, and one honest-looking line fixed.** The owner's `!update status` confirmed all three preconditions: the S127 code running, the in-process check firing 2 minutes after boot exactly as configured, and `Restart route: clean exit → systemd restarts me. No sudo involved.` The same output showed `2 commits behind` above `Last check: 4 minutes ago — Already on the latest version` — both true at their own moments, but reading as a self-contradiction. The stale check is now labelled as stale, with the next check's time; the marker is narrow enough that a failed last run is never softened into it. |
