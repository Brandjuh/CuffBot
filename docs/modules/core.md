...
- Replies that are ephemeral as a slash command are split by INTENT on the text path (S50): genuinely **private** output (rap sheets) goes to the user's **DMs**; **noise-only** ephemerals (game claims, cooldown notices — marked `textInChannel` by their command) answer right in the channel as a no-ping reply. Only important things reach DMs (owner rule). **When a DM fails (S46):** only Discord error 50007 is reported as a DM refusal — the note names the real fix (the server's **Privacy Settings → Direct Messages** toggle, which is per-server and separate from global DM settings, or an unblock). Every other failure is reported as the bot's own problem and logged with its error code, never blamed on "closed DMs".
