...
| Session | Change |
|---|---|
| S134 | **Times are Discord timestamps.** A job on cooldown reads `⏳ ready <t:…:R>` in the job board's embed. The select option beside it keeps the plain `⏳ 2h 15m` — Discord does not render timestamp markup inside select options, so the row carries both forms (`readyAt` and `unavailable`). Cooldown *lengths* (`cooldown 30m`, `takes 2m`) are durations, not moments, and stay as they were. |
