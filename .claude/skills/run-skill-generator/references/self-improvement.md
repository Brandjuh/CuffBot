...
## The world outranks the code

**An owner sentence about the WORLD invalidates the queue; re-read everything in its light before acting.** For 136 sessions "the bot" had exactly one referent, so every report auto-attached to it. Then the owner said *"we gebruiken exclusief Red bot nu"* — and three sessions of recent work changed meaning retroactively: a live bug report had been about the OTHER bot (S136 shipped a correct fix to the wrong codebase), a "rebuild 1:1" milestone became true by construction, and a "screening" request had meant the new platform itself. The signal was present a full session early ("van de Red bot, niet van de node") and was misread as a statement about code because the world-model was treated as a constant. Rule: when an owner message names a platform, deployment, or tool as *what we use now* — or a report's target is ambiguous between two systems — update the world-model FIRST, then re-derive what every queued item and open report means under it. `STATE.md`'s phase line is where the world-model lives; changing it is a session's most important write.
