...
**The flat shape (S93+, for commands that are not families):** `export default { command: { name, aliases?, description, emoji?, permission?, args, run(ctx, values) } }` — a group without subcommands, dispatched by `src/core/prefix/command.js`. Use it whenever a command does exactly one thing (`!badge`, `!daily`, `!rapsheet`): forcing those into groups would rename what the precinct types daily, and the flat shape already shares the group's ctx, arg types, permission gate and crash handling. Arg specs additionally take `min`/`max` (numbers) and `maxLength` (strings), and specs declared *after* a greedy arg are claimed from the end of the line — that is what retired the adapter's per-command `textGreedyArg` hint. `ctx.typing()` replaces `deferReply()`: a message command has no 3-second deadline, so slow work shows the typing indicator and answers once.
