...
- **Degrade vs disable under a missing intent (S38):** when an intent is off, decide per feature whether it *degrades* (message XP without content: less info, still fair) or must *disable entirely* (the STOP-POLICE hunt without Message Content: the bot can't hear the answer, so spawning would be an unwinnable game — a bug wearing a feature's clothes). Disable-outright features must say exactly why in their own config/status command. Keep the harmless parts running (activity tracking) so enabling the intent activates the feature instantly.
- **Multiple privileged intents → attempt cascade (S34):** once a second privileged intent enters the picture (MessageContent + GuildMembers), one bespoke fallback per intent stops scaling. `src/index.js` walks an `ATTEMPTS` table over the combinations (most capable first), retrying login without whatever the portal refuses until one succeeds, and sets one per-feature flag per intent (`client.messageContentAvailable`, `client.memberEventsAvailable`). Surface each flag *inside Discord* where the owner will actually look — `/radio-check` plus the dependent feature's own config/status command — naming the exact portal switch; a toggle only the owner can flip must be discoverable without SSH.
