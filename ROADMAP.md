...
- [ ] **M16.7 — Guess the candy** 🍬 *(AAA3A; small + assets)* — silhouette speed-quiz, 5–23 answer buttons, first correct click wins with elapsed time. Gate: verify the source repo license before bundling the 46 PNGs; else re-theme with our own zero-dep silhouettes (mugshot variant).
- [ ] **M16.8 — Rollout** 🎲 *(AAA3A; small-medium)* — lobby (2–50), rounds of 25 numbered buttons (30 s secret pick), the rolled number and non-pickers are eliminated, numbers stay disabled, last player wins prize 2500 🍩 (code default; 1000–50000 knob), leaderboard. Cog's tie-path crash NOT ported (tie embed instead).
- [ ] **M16.9 — Memory** 🧠 *(AAA3A; small-medium)* — single-player pairs grid (3x3/4x4/5x5, emoji pool of 12, center blank on odd grids), 1 s red flash on mismatch, decayed prize max(int((base − 5·s − 15·wrong)·(n/5)),0) with base 5000 scaled 1/3 / 2/3 / 1, optional loss cap, leaderboard. Cog's double-counted games bug not ported.
- [ ] **M16.10 — Wordle** 🟩 *(AAA3A; medium)* — typed guesses vs a dictionary (invalid = free retry), emoji-grid render replaces Pillow, EN lists first (length 4–11, attempts 5–10), per-member stats + guess distribution. Faithful naive yellow rule; the hardcoded-6 loss bug fixed to respect max_attempts (recorded deviation).
- [ ] **M16.11 — Hammertime** 🔨 *(Dumb-Cogs; medium)* — per-user timezones (reuse the S44 picker), natural-language time → all 7 Discord timestamp styles, member-list mode, optional auto-convert listener. Hand-rolled relative parser; simplified absolute parser (fidelity risk documented).
- [ ] **M16.12 — Heist** 💰 *(maxcogs; LARGE, staged)* — 25 timed heists with cooldowns, tools/shields (single-use), debt + 20% tax, police rolls + heat (+2%/heat, decay 1/2 h), jail + bail (+15% tax), materials → 28 craft recipes, levels 1–120 (+0.5%/level success cap +20%), crew robbery (4 × level ≥20, split pot). Stages: data tables → pure resolveHeist(rng) → views → restart-surviving scheduler.
- [ ] **M16.13 — City crime RPG** 🌃 *(CalaMari; LARGE, staged)* — the /crime underworld: 5 crime types with events (96), streaks (+5%/success cap +25%), jail/bail/jailbreak (35% base, +30% fail), 46 random scenarios, black market, 6-category leaderboard. Slices: non-targeted crimes → events → jail suite → targeted crimes → streaks → scenarios → market → boards.
