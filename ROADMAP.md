...
- [ ] **M26.2 — Connect4 → the `minigames` cog** (2 sessions) — the cog is **two games plus a shared frame**: Connect 4 (416 lines) and Tic-Tac-Toe (210), economy staking, per-member stats, a sortable leaderboard, one game per channel, thread support, replaceable inactive games, and invite/rematch/replace views (an opponent **accepts** before play).
  ...
  **Sliced panel-first (skill 0.5.38 — the rule this whole milestone exists because of):**
  - [ ] **M26.2a** — the shared frame (`Board`, `find_lines`, `try_complete_line` ported token-for-token), Connect 4 rules, the cog's heuristic AI, and **the panel**: invite → accept → play → rematch, one game per channel, stale games replaceable by anyone. Playable PvP and vs the bot. The old `connect4` module stays until 26.2b so nothing regresses mid-way.
  - [ ] **M26.2b** — Tic-Tac-Toe on the same frame, donut staking, per-member stats, the sortable leaderboard, `!minigames` admin config, and **delete `src/modules/connect4/`**.

- [ ] **M26.3 — City crime → panel-driven** (2 sessions) — the source's `crime/views.py` is 2,000+ lines: `MainMenuView`, `CrimeListView`/`CrimeView`/`CrimeButton`, `BailView`, `JailOptionsView`, `TargetSelectionView`, `BlackmarketView`, and **`CrimeAttemptView` with a live `Bail Out!` button during the attempt** — a decision the player makes mid-crime, which is missing *gameplay*, not missing decoration. Ours has zero components. **Keep the S89–S92 engine**: the tables, resolver, streaks, jail suite, 46 scenarios, market and boards were machine-diffed against the source and are correct. This is a presentation-layer rebuild.
- [ ] **M26.4 — Heist → the remaining seven panels** (1–2 sessions) — the source has `HeistSelectionView`, `ShopView`, `EquipView`, `CraftView`, `CrewLobbyView`, `HeistConfigView`, `ItemPriceConfigView`, `EventView`; we built **only the crew lobby**. Not reported by the owner and less severe than city, but the same mistake at smaller scale.
