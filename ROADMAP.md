...
- [x] **M14 — Goal tracker** 🎯 *(S103 — module `goals`)* — the scope was never narrowed down, so the module answers **both** readings with one structure: **precinct goals** (progress bars, 25/50/75/100% milestone announcements, Manage Server) and **personal goals** (up to ten each, progress logging, a board of who has finished most). Precinct goals may **count themselves** via `track:members` / `track:boosts`, read straight off the guild so there is no counter to maintain and nothing to rebuild after a restart. Milestones are recorded in the same write as the progress, which is what makes the 15-minute sweep safe to run forever.
...
- [x] **M16.14 — Mafia** 🕴️ *(AAA3A; VERY LARGE — **split out into its own milestone M24** after the S104 survey found 11,927 lines, and built there: M24.1 in S105, M24.2 in S108)* — Werewolf-class night/day engine (weighted votes, defense/judgement, win conditions), secret role DMs. See § M24 for the slicing and what remains.
...
- [x] **M17.2 — Convert the worst offenders** (S70) — every config command is a group: `!selfroles`, `!memorial`, `!hunting`, `!logbook`, `!claims`, `!economy`, `!xp`, `!ai`, `!birthday`, `!chatstarter`, `!starboard`, `!welcome`, and `!channellist` (absorbed `channel-list-config`). Retired `-config` names live on as group aliases (loader-registered). Framework grew group `aliases` + the `postable` channel-arg flag (S55 rule in one place).
- [x] **M17.3 — Convert the rest + retire the legacy path** *(S93–S96)*; patrol wizard returns as `!patrol wizard` (public message + owner-gated components); full docs sweep rides along per converted module. Sliced, since it is 45 commands:
...
## M24 — Mafia (surveyed S104; M24.1 built S105, M24.2 built S108)
...
- [x] **M24.1 — Classic, 5+ players** *(S105 — module `mafia`)* — the four Classic cards (Boss / Medic / Detective / Officer) for 5–20 players, the full night → day → vote → trial state machine, pure with injected `random` and `now`. Private per-player pickers via ephemeral components (the S98 non-originator rule at its sharpest — a table is a public message whose every meaningful press is secret); role reveal and investigation by DM, with a channel *pointer* and never the content if a DM fails. The parity win rule is **stated explicitly**, because the cog only implies it. Games are RAM-only (S71's call); stats persist.
- [x] **M24.2 — The next tier of roles** *(S108 — owner: "voer het uit, ik vroeg er specifiek om, server heeft genoeg mensen")* — nine more cards (Enforcer, Framer, Vigilante, Commissioner, Tail, Private Eye, Distraction, Executioner, Jester) plus the **Crazy** and **Chaos** modes with the cog's per-player-count tables transcribed verbatim. Ordered night resolution (block → frame → protect → kill → read) with a visit log only the Tail sees. Neutrals win **personally without ending the game**, which is why `wonAs` exists alongside `winner`. Fixed a real S105 bug on the way: succession lived inside `resolveNight`, so a *lynched* Boss left an Enforcer who could never shoot.
- [ ] **M24.3 — Anomalies, achievements, the long tail** (open-ended) — **deliberately unscheduled, and its gate is an owner decision, not a missing session.** Build it only once the precinct is actually playing mafia often enough to want a long tail; until then it is content for a game with no evidence of demand. Nothing in M24.1/M24.2 changes if the answer arrives later.

**What the owner already answered:** this is a **live, multi-hour game needing 5+ people online at once** — every other M16 game works with one or two players — and the owner scheduled it anyway on 2026-07-26 ("server heeft genoeg mensen"). That question is settled; the open one is only whether the long tail is wanted.
...
- [x] **M21 — Speech to text** 🎙️ *(S101 + S102)* — transcribe voice memos and voice-chat audio to **English**.

  **✅ OWNER DECISION (2026-07-26), asked before any code was written and binding for both halves:**
  - **Backend: Groq / Whisper.** The `GROQ_API_KEY` is already an expected env var (detective, S-era M9), so this adds **no new npm dependency** — Groq's audio endpoint is a plain multipart HTTPS POST and Node ≥18 has `fetch`, `FormData` and `Blob` built in. Rejected: a local whisper.cpp (a system binary + model file the owner would have to install by hand, which breaks the self-update property, and `base`-grade quality on Pi hardware) and Gemini (works, but Groq is real Whisper and more predictable for transcription).
  - **Scope: both halves — voice memos AND live voice chat.** The owner explicitly chose to include live voice knowing it needs `@discordjs/voice` and its encryption dependency, native-build risk on the Pi, continuous CPU/bandwidth, and that the bot is recording everyone in the channel.

  Split into two milestones because the dependency footprint is the dividing line:

- [x] **M21.1 — Voice memos** 🎙️ *(S101)* — a posted voice message or audio attachment is transcribed to English in-channel. **Zero new dependencies:** the file is downloaded over HTTP and POSTed to Groq; no voice gateway is involved at all.
  *Accept when:* eligibility (is it audio, is it in scope, is it under the size cap) and reply formatting are pure `lib/` logic with tests and an injectable `fetch` — **no network in the suite**; the provider call degrades honestly when no key is configured or Groq refuses; auto-transcription is per-guild configurable (on/off + channel scope) with a manual command for anything out of scope; and output defaults to **English regardless of the spoken language** (the owner's literal request — Groq's translation endpoint, with a knob for same-language transcription).

- [x] **M21.2 — Live voice chat** 🎧 *(S102)* — the bot joins a voice channel, captures each speaker's audio, and posts English transcripts to a text channel. **The first feature with dependencies beyond discord.js.**
  *Accept when:* the deps are chosen for a Pi that runs `npm ci` unattended — **prefer pure-JS/WASM over anything that needs a compiler**, and capture Opus packets straight into an Ogg container (Groq accepts Ogg/Opus) so no audio decoder is needed at all; joining/leaving is explicit and owner/Manage Server gated; the channel is told it is being recorded, because it is; per-speaker chunking is silence-based and the chunking rule itself is pure and tested with injected timings; and the update gate still passes on the Pi with the new dependencies installed.

---

## M25 — Owner requests, 2026-07-26 (all delivered)

These arrived in chat during S105–S111 rather than as a batch, so they were built directly. Recorded here after the fact (S112) because a milestone that exists only in `SESSION_LOG.md` is invisible to the next session planning its work — and the roadmap is where "what is left" gets read.

- [x] **M25.1 — No hyphenated command names** *(S106)* — owner: *"Pas alle commando's van alle modules aan, zodat we geen `!command-subcommand` hebben dit werkt niet prettig. Kijk naar hoe Red Discord bot dit soort dingen doen."* 19 hyphenated commands became 0; all 19 retired spellings still resolve as aliases. Three framework additions made it behaviour-preserving: `invokeWithoutSubcommand` (Red's `invoke_without_command`, so bare `!trivia` still starts a round), `!group help` reserved so a group whose bare form runs something can still list itself, and `permission: null` on a subcommand dropping the group's gate so public readers can live inside admin groups.
- [x] **M25.2 — Full-repo audit** *(S107)* — owner: *"ga je alles nakijken op vreemde dingen"*. Twenty-four checks. The headline finding was a measurement rather than a defect: a real clone + `npm ci` + `npm test` is green in 2 seconds with no compiler, so S102's first-ever dependency addition will pass the Pi's update gate.
- [x] **M25.3 — A permanent help panel** *(S109)* — owner: *"Ik mis nog steeds een help waarbij ik een paneel heb met categorieen."* `!help panel [#channel]` posts a standing category panel (S98's buttons were verified working first, so this is the *published-post* reading of the request, not a repair). A panel has no originator, so every press answers privately with that member's own filtered roster — the S98 non-originator rule with the originator removed.
- [x] **M25.4 — Auto-join voice for transcription** *(S110)* — owner: *"Zodra iemand een VC joined wil ik dat de bot automatisch erbij gaat zitten. De voice en text kanalen hebben dezelfde naam."* The bot follows anyone into a voice channel and transcribes into the text channel with the matching name, then leaves when the room empties. "The same name" is the whole problem and is pure, normalised, three-pass matching in `transcribe/lib/pairing.js`; an ambiguous near-miss is refused rather than guessed, and the fallback is the voice channel's own built-in chat.
...
- [x] **M26.1 — Audit every game against its source** *(S115 — `docs/porting/S115-game-interaction-audit.md`)* — objective signal: count the source cog's `discord.ui` references against our module's component files. **Nine of thirteen games match their source's interaction model.** Two diverged, and they are the two largest ports.
- [x] **M26.2 — Connect4 → the `minigames` cog** — **COMPLETE** (2 sessions, as estimated) — the cog is **two games plus a shared frame**: Connect 4 (416 lines) and Tic-Tac-Toe (210), economy staking, per-member stats, a sortable leaderboard, one game per channel, thread support, replaceable inactive games, and invite/rematch/replace views (an opponent **accepts** before play).

  ✅ **OWNER DECISIONS (2026-07-26), asked before any code was written:**
  - **Full replacement, including the cog's own bot opponent.** The S100 negamax + alpha-beta solo AI and its three difficulty levels are **retired**. The owner chose "alles van de cog, ook hun bot" with the trade-off stated: the cog's opponent is a scoring heuristic (win → block → count 3s and 2s → penalise dangerous setups → prefer centre, 5% randomness) and is measurably weaker. Port that heuristic **verbatim**, including the randomness, rather than quietly keeping ours behind it.
  - **Everything comes along:** Tic-Tac-Toe, **donut staking**, and **stats + leaderboard**. Staking uses the cog's defaults — bet `100`, winner takes a random `400`–`600` — through the existing `adjustBalance` economy seam, withdrawn up front and **refunded if the game is cancelled before it starts**.

  **Sliced panel-first (skill 0.5.38 — the rule this whole milestone exists because of):**
  - [x] **M26.2a** *(S116 — module `minigames`)* — the shared frame (`Board`, `find_lines`, `try_complete_line` ported token-for-token), Connect 4 rules, the cog's heuristic AI, and **the panel**: invite → accept → play → rematch, one game per channel, stale games replaceable by anyone. Playable PvP and vs the bot. ⚠️ **Two plan corrections, both forced by reality:** the loader rejects duplicate command names, so "the old module stays until 26.2b" was never available and `connect4` was deleted here; and since that would have removed the scoreboard for a session, **stats were pulled forward** from 26.2b, reading and writing the old `connect4Stats` key so existing records carry over.
  - [x] **M26.2b** *(S125)* — Tic-Tac-Toe on the same frame (its board IS the buttons, and stays on screen when the game ends), donut staking (100 in on accept, 400–600 out, refunded on a tie or a pre-move cancel, never charged to a bot), the sortable `!gameleaderboard`, and the `!minigames` config group. `!connect4 stats`/`board` removed in favour of one shared surface — both games always wrote to one set of counters. **Added beyond the cog:** `betvsbot`, because the cog's stake-against-its-own-bot is a +300–500 faucet against a heuristic with no lookahead; the cog's behaviour stays the default. *(Stats and the module deletion moved into 26.2a — see above.)*

- [x] **M26.3 — City crime → panel-driven** — **COMPLETE (S133).** 26.3a (S122): the panel, the crime picker, jail's buttons and the missing Bail Out mechanic. 26.3b (S124): the crime narrated beat by beat with the bail flag checked before each (a four-event bank job now offers six chances to walk away, not one), the mark picker with its random roll and last-victim rule, and market/leaderboard as panel buttons with a Back. **26.3c (S133): `MainMenuView` — the hub, and the item that made "COMPLETE" wrong for eleven sessions.** The inventory below names eight source views; S122/S124 built seven. `MainMenuView` is `[p]city`, which S122 had removed as an alias of `crime` while reserving the name "for the hub when it exists" — so the gap was not merely unbuilt, it was a command the owner had typed since S90 that now answered with **silence** (the router drops an unknown command without a word). Closing a milestone against a list is worth nothing if nobody diffs the list; the guard for that is now in `test/docs-consistency.test.js`. (3 sessions against an estimate of 2) — the source's `crime/views.py` is 2,000+ lines: `MainMenuView`, `CrimeListView`/`CrimeView`/`CrimeButton`, `BailView`, `JailOptionsView`, `TargetSelectionView`, `BlackmarketView`, and **`CrimeAttemptView` with a live `Bail Out!` button during the attempt** — a decision the player makes mid-crime, which is missing *gameplay*, not missing decoration. Ours has zero components. **Keep the S89–S92 engine**: the tables, resolver, streaks, jail suite, 46 scenarios, market and boards were machine-diffed against the source and are correct. This is a presentation-layer rebuild.
- [x] **M26.4 — Heist → the remaining seven panels** — **COMPLETE.** 26.4a (S126): the four player-facing panels — job board, shop, equipment rack, crafting bench — paged, with selects that act, and the board folding the level bonus into the success band the static list never showed. 26.4b (S130): `HeistConfigView` → `!heist tune`, `ItemPriceConfigView` → `!heist pricing`, `EventView` → `!heist event`, all gated on Manage Server twice (subcommand and pump). (2 sessions, as estimated) — the source has `HeistSelectionView`, `ShopView`, `EquipView`, `CraftView`, `CrewLobbyView`, `HeistConfigView`, `ItemPriceConfigView`, `EventView`; we built **only the crew lobby**. Not reported by the owner and less severe than city, but the same mistake at smaller scale.
