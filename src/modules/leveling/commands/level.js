// S93 (M17.3 slice A): converted to the flat { command } shape. Open to
// everyone — a service record is public in this precinct.
...
export default {
  command: {
    name: 'level',
    description: 'Show a member’s XP, rank, and progress to the next rank.',
    emoji: '🎖️',
    ...
    async run(ctx, { target: requested }) {
      ...
      if (target.bot) {
        ...
        await ctx.reply('🚫 Bots don’t earn XP — K9 units are paid in treats.');
        return;
      }
      const member = await ctx.guild.members.fetch(target.id).catch(() => null);
      if (!member) {
        await ctx.reply(`🚫 ${target} is not in the precinct.`);
        return;
      }
      ...
      const record = ensureSeeded(ctx.guild.id, member, ladder, config);
      const xp = record.xp;
