...
| Tests green | `npm test` | 210/210 pass as of S16 |
...
- **Leveling (S16):** module `leveling` — CuffBot's own XP system, **replacing the old leveler bot** (owner decision S15/S16). Message XP (cooldown-gated; needs only the MessageCreate *event*, works without Message Content) + voice XP (60 s sweep; anti-farm: no AFK channel, ≥2 humans, not self-deafened, no bots; `GuildVoiceStates` intent added to the base set). Thresholds `round(baseXp·N^1.6)` mapped position-based onto the academy ladder; **promote-only** auto rank sync (never demotes) with audit reasons + no-ping announcements. **Seeding (owner requirement S16): first sight of a member with a rank role seeds their XP at that rank's threshold floor — existing members never restart at 0; rankless members start at 0; seeding runs at most once (`seededFromRank` stored).** `/level` (progress card), `/leaderboard`, `/xp-config` (admin). Academy gained the interaction-free `ladderForGuild(guild)` seam. Manual `leveling.md`.
...
- **Tests:** 210 via `node:test` — config, env loader, loader integrity, core lib, diagnostics, prefix parse/adapter (incl. role resolution), help, enforcement lib + GIF, academy ladder + commands, dispatch, patrol screen/event/commands, leveling (pure math, seeding, service, commands, both events), and command smokes with fake interactions.

## Resume point

**The base (M1–M8) plus leveling is complete: 8 modules, 27 commands, 210 tests, dual invocation, self-update, audited.** Next: **M9 — AI conversation**, now unblocked by owner decisions (S16, recorded in ROADMAP M9):
